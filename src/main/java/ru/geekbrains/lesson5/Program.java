...
public class Program {
    ...
    public static void main(String[] args) throws IOException {
        ...
        copyFileReserved("C:\\Users\\Stas\\Desktop\\Учеба GB\\Programmist\\Java_Core\\lesson5");
    ...
    /**
     *
     * @param path путь до папки, откуда необходимо скопировать файлы
     * @throws IOException
     */
    private static void copyFileReserved(String path) throws IOException {
        String pathBackup = "./backup";
        File dirBackup = new File(pathBackup);
        dirBackup.mkdir();
        ...
        if (dir.isDirectory()) {
            for (File item : dir.listFiles()) {
                if (item.isFile()) {
                    File newFile = new File(pathBackup + "//" + item.getName());
